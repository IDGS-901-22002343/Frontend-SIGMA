...
export default function MantenimientosList() {
  ...
  }, [cargarMantenimientos, clearError]);
  ...
  const filtrados = useMemo(() => {
    ...
    if (!t) return mantenimientos || [];
    return (mantenimientos || []).filter(m =>
      m.vehiculoInfo?.toLowerCase().includes(t) ||
      m.numeroEconomico?.toLowerCase().includes(t) ||
      m.tipoNombre?.toLowerCase().includes(t) ||
      m.proveedorNombre?.toLowerCase().includes(t)
    );
    ...
    <div style={{ padding: "32px 36px", fontFamily: "'Barlow', sans-serif", minHeight: "100%" }}>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ margin: "5px 0 0", fontSize: 14, color: "#9ca3af" }}>
          Registro de servicios y reparaciones de vehículos
        </p>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ fontSize: 15, fontWeight: 600, color: "#111827" }}>Listado de Mantenimientos</span>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input
                type="text"
                placeholder="Buscar por vehículo, tipo, taller..."
                value={busqueda}
        ...
        <div style={{ overflowX: "auto", width: "100%" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "auto" }}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={7} style={{ padding: "52px 0", textAlign: "center", color: "#9ca3af", fontSize: 14 }}>Cargando mantenimientos...</td></tr>
              ) : filtrados.length === 0 ? (
                <tr><td colSpan={7} style={{ padding: "52px 0", textAlign: "center", color: "#9ca3af", fontSize: 14 }}>No hay mantenimientos registrados</td></tr>
              ) : filtrados.map(m => (
                ...
                >
                  ...
                  <td style={{ padding: "15px 20px", textAlign: "center", borderBottom: "1px solid #f7f7f7" }}>
                    <div style={{ display: "flex", gap: 6, justifyContent: "center" }}>
                      ...
                      />
                      <Btn onClick={() => onEliminar(m.idMantenimiento)} label="Eliminar"
        ...
        {filtrados.length > 0 && (
          <div style={{ padding: "13px 24px", borderTop: "1px solid #f5f5f5", fontSize: 12, color: "#9ca3af", textAlign: "center" }}>
            Mostrando {filtrados.length} de {mantenimientos?.length || 0} mantenimientos
          </div>
        )}
      </div>
        ...
        <MantenimientoForm
          ...
          onSubmit={async (vals) => {
            try {
              ...
              setLocalErr("");
              clearError();
              if (editTarget?.idMantenimiento) {
                await actualizarMantenimiento(editTarget.idMantenimiento, vals);
              } else {
                await crearMantenimiento(vals);
              }
              onCerrar();
            } catch (e) {
